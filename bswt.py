...
#Поиск компонент. 
def SearchComponent(image, center, mask, cntrimg, original, buff):
    component = [center]
    #q = Queue()
    #q.put(center)
    ...
    closed_points = 0
    open_points = 1
    while open_points >= closed_points:#len(q) > counter:#not q.empty():
        #point = q.get()
        #point = q[closed_points]
        point = (X[closed_points], Y[closed_points])
        closed_points += 1
        #print point        
        for i in xrange(-1, 2):
            for j in xrange(-1, 2):
...
def Association(gray, contour, swimage, debug_components=False, ser=''):
    ...
    components = []
    X = np.zeros((gray.shape[0] * gray.shape[1]), dtype=np.uint16)
    Y = np.zeros((gray.shape[0] * gray.shape[1]), dtype=np.uint16)
    for j in xrange(gray.shape[1]):
        for i in xrange(gray.shape[0]):
            if (mask[i, j] == 0) and (swimage[i, j] < CC_B):#CC_B = inf - "барьер"
                res = SearchComponent(swimage, (i, j), mask, contour, gray, (X, Y))
